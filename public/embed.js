...
(function () {
  ...
  async function init(currentScript) {
    const widgetUrl = `${apiUrl}widgetData/${uuid}`;
    ...
    //   // Loop through all script elements and remove them
    ...
    //   });
      ...
      containerDiv.innerHTML = bodyContent;

      // Append the container to the script element's parent node
      currentScript.parentNode.insertBefore(containerDiv, currentScript.nextSibling);
	  ...
	  const cssStyleLinks = Array.from(doc.getElementsByTagName('style'))
      ...
      const js = Array.from(doc.getElementsByTagName('script'));
      // console.log("jsjs", js.map(j => j.sr));
      ...
      console.error("Failed to load widget data:", response.statusText);
    }
    // Example: Hide loader after 3 seconds (use hideLoader when you're ready to remove it)
    setTimeout(hideLoader, 3000);
  }

  // Pass the script element to the init function
