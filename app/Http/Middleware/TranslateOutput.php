...
class TranslateOutput
{
    ...
    //             $fullMatch = $matches[0];
    //             $text = isset($matches[3]) ? trim($matches[3]) : ''; // Trim whitespace
    
    //             // Skip empty or non-translatable content
    //             if ($text === '' || is_numeric($text) || strpos($fullMatch, 'data-no-translate') !== false) {
    //                 return $fullMatch;
    //             }
    ...
    //             $translatedText = $translations[$normalizedText] ?? null;
    //             if (!$translatedText) {
    //                 // Fallback to dynamic translation if not found
    //                 $translatedText = app('App\Services\TranslationService')->translate($text, $language);
    
    //                 // Save new translation
    //                 $translations[$normalizedText] = $translatedText;
    ...
    // protected function saveTranslation($filePath, $translations)
    // {
    //     ksort($translations); // Sort translations for readability
    //     $content = "<?php\n\nreturn " . var_export($translations, true) . ";\n";
    //     file_put_contents($filePath, $content);
    // }
