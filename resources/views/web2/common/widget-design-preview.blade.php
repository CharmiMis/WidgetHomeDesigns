<!-- Modal -->
<div class="modal fade gs-image-editing-background" id="modalImagePreview" tabindex="-1" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    {{-- <div class="modal-dialog gs-modal-container">
        <div class="modal-content gs-modal-content">
            <button type="button" class="gs-modal-close" data-dismiss="modal">
                <img src="{{ asset('webWidget/images/gs-close-icon.svg') }}">
            </button>
            {{-- <div class="gs-image-editing-slide-outer">
                <div class="gs-image-editing-slide">
                    <div class="gs-image-editing-slide-row">
                        <div class="inked-painted">
                            <img src="" id="mip_before"
                                class="inked" alt="results when using beautiful redesign with ai">
                            <div class="colored" id="mip_after"
                                style="" title="ai beautiful redesign before generating">
                            </div>
                        </div>
                    </div>
                </div>
            </div> --}}
            <div class="modal-dialog  gs-image-editing-container">
                <div class="modal-content gs-image-editing-content">
                    <button type="button" class="gs-modal-close" data-dismiss="modal">
                        <img src="{{ asset('webWidget/images/gs-close-icon.svg') }}">
                    </button>
                    <div class="gs-image-editing-slide-outer">
                        <div class="gs-image-editing-slide">
                            <div class="gs-image-editing-slide-row">
                                <div class="ba-slide-outer">
                                    <div class="inked-painted">
                                        <img src="" id="mip_before" class="inked"
                                            alt="results when using beautiful redesign with ai">
                                        <div class="colored" id="mip_after" style=""
                                            title="ai beautiful redesign before generating">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        {{-- <div class="btn-wrap">
                            <button class="prev-btn"><span><img
                                        src="{{ asset('webWidget/images/gs-slide-arrow.svg') }}"></span>Prev</button>
                            <button class="next-btn"><span><img
                                        src="{{ asset('webWidget/images/gs-slide-arrow.svg') }}"></span>Next</button>
                        </div> --}}
                    </div>
                </div>
            </div>
        {{-- </div>
    </div> --}}
</div>
